...
display_freq() {
  if [ -f /sys/kernel/debug/clk/bimc_clk/measure ]; then
    echo "BIMC cur_freq: " $(< /sys/kernel/debug/clk/bimc_clk/measure)
  elif [ -f /sys/kernel/debug/clk/bimc_clk/clk_measure ]; then
    echo "BIMC cur_freq: " $(< /sys/kernel/debug/clk/bimc_clk/clk_measure)
  elif [ -f /sys/kernel/debug/clk/measure_only_bimc_clk/clk_measure ]; then
    # Napali
    echo "BIMC cur_freq: " $(< /sys/kernel/debug/clk/measure_only_bimc_clk/clk_measure)
...
bimc_freq_switch() {
  ...
  for REQ_KHZ in ${FREQ_LIST}; do
    ...
    elif [ -f /sys/kernel/debug/rpm_send_msg/message ]; then
...
elif [ $platform = "atoll" ]; then
  bimc_scaling_freq_string="1804800 1555200 1017600 768000 547200 200000"
elif [ $platform = "bengal" ]; then
  bimc_scaling_freq_string="1804800 1555200 1353600 1017600 768000 547200 451200 300000 200000"
else
...
scaling_freq_len=0
for freq in $bimc_scaling_freq_string
do
    if [[ "$freq" != "" ]]; then
        scaling_freq_len=$(($scaling_freq_len+1))
    fi
done
...
loop_random_bimc_freq_switch() {
  while [ 1 ] ; do
    fix_freq_select=`getprop vendor.oplus.dram.freq.option`
    if [[ "$fix_freq_select" != "" ]]; then
        #fix freq tune
        configvalue=`echo ${fix_freq_select} | sed -e 's/^[ \t]*//g' -e 's/[ \t\r\n]*$//g'`
        case $configvalue in
             "1" )
                FREQ_LIST=$highfreq
                bimc_freq_switch
...
if [ -f /sys/kernel/debug/rpm_send_msg/enable ]; then
  echo 3735928559  > /sys/kernel/debug/rpm_send_msg/enable
